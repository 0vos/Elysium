...
- 底部有一个**模型选择按钮**，点它在“立方体 / 球体”之间切换（按钮上显示当前模型）。
- **点平面放置**：轻点扫描出的平面，把当前选中的模型放上去（没检测到平面时会放到相机前方）。
- **手势控制**：轻点已放置的模型选中它，然后单指拖动移动、双指捏合缩放、双指旋转。
- 把立方体和球体拖到一起，两者**碰撞时会变绿**，顶部提示碰撞检测成功；分开后恢复原色。
...
- 屏幕 UI 用 OnGUI 绘制，标签用 ASCII 英文（避免设备缺少中文字体时显示为方块）。
- 输入走新的 Input System（`EnhancedTouch` + `Mouse`）；工程 `activeInputHandler = 1`（纯新输入系统），旧的 `Input.touch` 等 API 收不到事件，不能用。
